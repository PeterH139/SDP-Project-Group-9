...
public class RobotRecogniser implements ObjectRecogniser {
	...
	public void processFrame(PixelInfo[][] pixels, BufferedImage frame,
		...
		if (leftBlueFirst) {
			...
			// Yellow Defender
			blueDef = searchColumn(pixels, debugOverlay,
						leftBuffer, dividers[0], true);
			yellowAtk = searchColumn(pixels, debugOverlay,
						dividers[0], dividers[1], false);
			blueAtk = searchColumn(pixels, debugOverlay,
						dividers[1], dividers[2], true);
			yellowDef = searchColumn(pixels, debugOverlay,
						dividers[2], frame.getWidth() - rightBuffer, false);
		} else {
			// In order, ltr: Yellow Defender, Blue Attacker, Yellow Attacker,
			// Blue Defender
			yellowDef = searchColumn(pixels, debugOverlay,
					leftBuffer, dividers[0], false);
			blueAtk = searchColumn(pixels, debugOverlay,
					dividers[0], dividers[1], true);
			yellowAtk = searchColumn(pixels, debugOverlay,
					dividers[1], dividers[2], false);
			blueDef = searchColumn(pixels,debugOverlay,
					dividers[2], frame.getWidth() - rightBuffer, true);
		}
	 ...
	 */
	private Position searchColumn(PixelInfo[][] pixels, BufferedImage debugOverlay, 
		...
		int obj = isBlue ? PitchConstants.BLUE : PitchConstants.YELLOW;
		...
		for (int row = topBuffer; row < VideoStream.FRAME_HEIGHT - bottomBuffer; row++){
			for (int column = leftEdge; column < rightEdge; column++){
				if (pixels[column][row] != null){
					if (vision.isColour(pixels[column][row], PitchConstants.GREEN)){
		...
		Position minX = new Position(VideoStream.FRAME_WIDTH+1,VideoStream.FRAME_WIDTH+1); 
		Position minY = new Position(VideoStream.FRAME_HEIGHT+1,VideoStream.FRAME_HEIGHT+1);
		for (Position p : greenPoints){
			int x = p.getX();
			int y = p.getY();
			if (x < minX.getX()) minX = p;
			if (x >= maxX.getX()) maxX = p;
			if (y <= minY.getY()) minY = p;
			if (y > maxY.getY()) maxY = p;
		}
		// For Debugging
		debugOverlay.getGraphics().drawLine(minX.getX(), minX.getY(), minY.getX(), minY.getY());
		debugOverlay.getGraphics().drawLine(minX.getX(), minX.getY(), maxY.getX(), maxY.getY());
		debugOverlay.getGraphics().drawLine(maxX.getX(), maxX.getY(), minY.getX(), minY.getY());
		debugOverlay.getGraphics().drawLine(maxX.getX(), maxX.getY(), maxY.getX(), maxY.getY());
	 ...
	 * @author Peter Henderson (s1117205)
	 */
	private float calculateAngle(PixelInfo[][] pixels, BufferedImage debugOverlay, Position pos, int margin){
		...
		for (int row = startRow; row < endRow; row++){
			for (int column = startColumn; column < endColumn; column++){
				...
				if (x2 + y2 <= r2 && inBounds && pixels[column][row] != null){
					if (vision.isColour(pixels[column][row], PitchConstants.GREY)){
